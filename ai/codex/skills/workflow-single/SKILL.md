...
例:
  workflow-single                        # 既存計画書から実行（worktree）
  workflow-single --plan                 # 計画書を作成してから実行
  workflow-single --branch               # ブランチ作成モードで実行
...
```
[既存計画書] → worktree/ブランチ作成 → 実装 → 自己レビュー → [確認] → コミットメッセージ出力
```
...
```
[ユーザー入力] → 計画書作成 → 承認 → worktree/ブランチ作成 → 実装 → ... → コミットメッセージ出力
```
...
```
[タスク説明] → worktree/ブランチ作成 → 実装 → 自己レビュー → [確認] → コミットメッセージ出力
```

## 実行フロー

```
worktree/ブランチ作成 → 実装 → 自己レビュー → [確認] → コミットメッセージ出力
```
...
github-worktree-create スキルのワークフローに従って実行する。
...
- デフォルトで worktree を作成する（--branch でブランチ作成に変更可能）
- コミットメッセージを出力する
- 機密ファイルを警告する
- Issue を作成しない
- 自動でコミット・プッシュしない
